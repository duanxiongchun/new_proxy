...
if threads > len(allowed):
    raise SystemExit(
        f"threads={threads} requires {threads} CPUs, but allowed cpuset has {len(allowed)}: {sys.argv[1]}"
    )
print(",".join(str(cpu) for cpu in allowed[:threads]))
PY
}

validate_threads_list() {
  for threads in $THREADS_LIST; do
    if ! [[ "$threads" =~ ^[0-9]+$ ]] || [ "$threads" -lt 1 ]; then
      echo "Invalid PERF_THREADS_LIST entry: $threads" >&2
      exit 1
    fi
    if [ "$threads" -gt "$ALLOWED_CPU_COUNT" ]; then
      echo "threads=$threads requires $threads CPUs, but allowed cpuset has $ALLOWED_CPU_COUNT: $ALLOWED_CPU_LIST" >&2
      echo "Set PERF_CPU_LIST or PERF_THREADS_LIST to match this host." >&2
      exit 1
    fi
  done
}
...
run_group() {
  ...
  if ! kill -0 "$CLIENT_PID" 2>/dev/null; then
    echo "Client daemon exited early for threads=$threads" >&2
    cat "$client_log" >&2
    exit 1
  fi
...
for threads in $THREADS_LIST; do
  ...
  worker_dump="$ARTIFACT_DIR/client_${threads}_dump.txt"
  ...
  if [ "$worker_count" -ne "$threads" ]; then
    echo "Expected $threads worker telemetry rows, got $worker_count in $worker_dump" >&2
    cat "$worker_dump" >&2
    exit 1
  fi
