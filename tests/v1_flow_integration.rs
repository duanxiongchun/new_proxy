...
use new_proxy::flow_plane::{
    bounded_flow_channels, DispatchOutcome, FlowMessage, FlowWorkerState, IoOwnerKey, IoRegistry,
    QuicFlowId,
};
...
fn v1_integration_session_owner_and_local_return_io_stay_stable() {
    ...
    let first = worker
        .handle_intercept(intercept, packet.clone(), QuicFlowId(7), tunnel)
        .unwrap();
    let repeated = worker
        .handle_intercept(intercept, packet, QuicFlowId(7), tunnel)
        .unwrap();
...
fn v1_integration_reverse_nat_restores_original_return_tuple() {
    ...
    let handled = worker
        .handle_intercept(intercept, original.clone(), QuicFlowId(7), tunnel)
        .unwrap();
...
fn v1_integration_icmp_reverse_nat_restores_identifier_and_keeps_reply_type() {
    ...
    let handled = worker
        .handle_intercept(intercept, original, QuicFlowId(7), tunnel)
        .unwrap();
    let translated = new_proxy::flow_plane::parse_flow_key(&handled.transmit.packet).unwrap();
...
fn v1_integration_same_interface_keeps_tunnel_and_intercept_queues_distinct() {
    ...
    let handled = worker
        .handle_intercept(intercept, ipv4_tcp_packet(10001), QuicFlowId(7), tunnel)
        .unwrap();
